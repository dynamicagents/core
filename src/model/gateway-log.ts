...
export type GatewayPhase = "turn" | "sub-agent" | "compaction" | "embed";
 ...
 * every task, so it has no task; a parent turn has no sub-agent run in scope.
 *
 * There is deliberately no field for the person who sent the turn. The
 * gatekeeper identity carries one, and the only thing keeping it out of an
 * account-retained log is that there is nowhere here to put it —
 * {@link gatewayLogFields} reads these keys and no others, so spreading a whole
 * identity in cannot leak one either.
 */
export interface GatewayCorrelation {
  agent?: string;
  taskId?: string;
  phase?: GatewayPhase;
  /** The sub-agent run this call belongs to, for a call made inside one. */
  runId?: string;
  /** The conversation this task belongs to — a channel, not a person. */
  contextId?: string;
}
...
export interface GatewayLogFields {
  ...
  /**
   * The one exact handle for a unit of work — a sub-agent run — in the Logs
   * API's `event_id` filter. Metadata cannot stand in for it: a filter on
   * `metadata.value` matches any key holding that value, so two keys holding the
   * same string are indistinguishable there.
   *
   * A task on its own is not a unit of work here: it spans however many turns it
   * takes to answer, so its calls share no single handle and none is offered.
   */
  eventId?: string;
}
...
export function gatewayLogFields(
  ...
  const { taskId, runId } = correlation;
  const eventId = taskId && runId ? `${taskId}:${runId}` : undefined;
