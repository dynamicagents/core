...
import type { HitlOption } from "@dynamicagents/g2a-protocol";
...
export interface TurnOutcome {
   ...
   *
   * Not the whole turn: the sentences before a tool call are the model narrating
   * what it is about to do, and they have already been pushed as progress by the
   * `onChunk` flush.
   */
  reply: string;
  ask?: PendingAsk;
...
/** The task a message was submitted under, from what Think stamped on it. */
export function taskIdOf(message: UIMessage): string | undefined {
  const metadata = message.metadata as
    | { turnMetadata?: { taskId?: unknown } }
    | undefined;
